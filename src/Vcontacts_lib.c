...
char   test_point_vcon(double ptX[], struct plane cont[], int NC, float rado, int planeA, int planeB, int planeC);
...
int    calc_region(struct atom PDB[], int PDBtot, int atomlist[], int numatoms,char vince_pdb[100]);
...
int Vcontacts(char* filename,gsl_matrix *mat)
{
  ...
  ca_index = (long int*)malloc(PDBtot*sizeof(long int));
  ca_recsize = 5*PDBtot;
  ca_rec = (struct ca_struct*)malloc(ca_recsize*sizeof(struct ca_struct));
  seed = (long int*)malloc(3*PDBtot*sizeof(long int));
  ...
  calc_region(PDB, PDBtot, PDBlist, PDBtot,filename);
  return(0);
}
		 ...
		 int NC, int NV, struct plane cont[], struct ptindex ptorder[])
{
  ...
  for(planeX=0; planeX < NC; ++planeX) {
	...
	if(poly[ptorder[planeX].pt[vi]].plane[2] == -1) { // on arc, calc pt. opposite
	  ...
	  if(test_point_vcon(temppt, cont, NC, rado, planeX, -1, -1) == 'Y') {
		...
		struct plane cont[], struct ptindex ptorder[], int atomzero)
{
  ...
  for(planeX=0; planeX < NC; ++planeX) {
    ...
    if(NP == 0) {
      if(test_point_vcon(centerpt[planeX].xi, cont, NC, rado, planeX, -1, -1) == 'Y') {
	cont[planeX].area = 2.0*PI*rado*(rado-centerpt[planeX].dist);
      }
    } else if(NP == 2) {  // only two contact points, check which part of arc
      if(test_point_vcon(centerpt[planeX].xi, cont, NC, rado, planeX, -1, -1) == 'Y') { // area is (cap - arc)
	cont[planeX].area = 2.0*PI*rado*(rado-centerpt[planeX].dist)
...
void save_areas(struct plane cont[], struct contactlist contlist[], int NC, int atomzero)
{
  ...
  if(numcarec > (ca_recsize-100)) {
    ca_recsize += 10000;
    ca_rec = (struct ca_struct*)realloc(ca_rec, ca_recsize*sizeof(struct ca_struct));
    if(!ca_rec) { 
      printf("memory allocation error\n"); 
      exit(1);
    }
...
struct atom *read_PDB(char *FILE_ptr, struct atom *PDB, long int *numPDBatoms)
{
  ...
  while (1) {
    ...
    if(PDBtot>=PDBarraysize) {
      PDBarraysize += 1000;
      PDB = (struct atom*)realloc(PDB, PDBarraysize*sizeof(struct atom));
      if(!PDB) {
	printf("memory allocation for PDB atoms failed\n");
	exit(1);
      }
...
void assign_radii(struct atom *PDB, long int PDBtot)
{
	...
	char   typenaame[11][5] = {"C3H0","C3H1","C4  ","N3H0","N3H1","N3H2","N4  ","O1H0","O2H1","S   ","DEF "};
	...
	while(1) {
		...
		if(oneline[0] != '#') {
			if(strncmp(&oneline[2], typenaame[recnum], 4)) {
			  //printf("in file '%s', expected atom type %s, line %d\n", filename, typenaame[recnum], linenum);  //** COMMENTED OUT BY RJN 28.04.2008
			  printf("in file '%s', expected atom type %s, line %d\n", radfilename, typenaame[recnum], linenum); //** ADDED BY RJN 28.04.2008
				exit(1);
			} else {
