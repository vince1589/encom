...
int    voronoi_poly2(struct atom PDB[], int atomzero, struct plane cont[], float rado, 
		     int NC, struct contactlist contlist[],struct big_global big_g);
...
int    calc_region(struct atom PDB[], int PDBtot, int atomlist[], int numatoms,char vince_pdb[100],struct big_global big_g);
...
int    index_protein(int PDBtot, struct big_global big_g);
void   assign_radii(struct atom *PDB, long int PDBtot);
void   save_areas(struct plane cont[], struct contactlist contlist[], int NC, int atomzero,struct big_global big_g,struct big_global *bla);
int    get_contlist4(struct atom PDB[], int atomzero, struct contactlist contlist[], 
		     int PDBtot, float rado, int dim, struct big_global big_g);
void   parse_commandline(int argc, char *argv[], char FILENAME[],struct big_global big_g);
void   save_seeds(struct plane cont[], struct vertex poly[], int NV, int atomzero,struct big_global big_g);
void   get_firstvert(struct plane cont[], int *planeA, int *planeB, int *planeC, int NC, int atomzero, struct big_global big_g);
...
int main(int argc, char *argv[])
{
  ...
  numcarec = 0;
  parse_commandline( argc, argv, FILENAME,big_g);
  ...
  big_g.seed = malloc(3*PDBtot*sizeof(long int));
  if((!ca_rec) || (!ca_index) || (!big_g.seed)) {
  ...
  big_g.dim = index_protein(PDBtot,big_g);

  // calc volumes for all protein atoms
  calc_region(PDB, PDBtot, PDBlist, PDBtot,FILENAME,big_g);
  printf("Here !!!!\n");
  return(0);
}
...
int calc_region(struct atom PDB[], int PDBtot, int atomlist[], int numatoms, char vince_pdb[100],struct big_global big_g)
{
  ...
  for(atomi=0; atomi<numatoms; ++atomi) {
    ...
    NC = get_contlist4(PDB, atomzero, contlist, PDBtot, rado, big_g.dim,big_g);

    NV = voronoi_poly2(PDB, atomzero, big_g.cont, rado, NC, contlist,big_g);
                     
    //printf("%5d%5s %4d%5s %c atomi=%d (out of %d) NC=%d NV=%d\n",                   // RJN 081008
    //   PDB[atomi].atomnum,PDB[atomi].atomname,PDB[atomi].resnum,                    // RJN 081008
    //   PDB[atomi].res,PDB[atomi].chn,atomi,numatoms,NC,NV);                         // RJN 081008
    surfatom = order_faces(atomzero, poly, centerpt, rado, NC, NV, big_g.cont, ptorder);    
    calc_areas(poly, centerpt, rado, NC, NV, big_g.cont, ptorder, atomzero);                
    save_areas(big_g.cont, contlist, NC, atomzero,big_g,&big_g);                                         
   printf("Numcarec bla:%d %d\n",numcarec,big_g.numcarec);
  ...
  } else if(big_g.planedef == 'R') {
    printf("#    -planedef R: radical plane\n"); // used as default
  } else if(big_g.planedef == 'B') {
  ...
  int vince_index = -1;
  int vince_i = -1;
  for(vince_i = 0; vince_i < 100;++vince_i) {
  	if(vince_pdb[vince_i] == '.') {vince_index = vince_i;}
  }
  
  printf("LE INDEX:%d\n",vince_index);
  vince_pdb[4] = '\0';
  sprintf(vince_temp,"vcon_out_%s.dat",vince_pdb);
		  ...
		  int NC, struct contactlist contlist[],struct big_global big_g)
{
 ...
 RESTART:
  ...
  add_vedge(big_g.vedge, 0, cont, planeA, planeB, planeC, poly, 0);
  add_vedge(big_g.vedge, 1, cont, planeB, planeC, planeA, poly, 0);
  add_vedge(big_g.vedge, 2, cont, planeC, planeA, planeB, poly, 0);
  ...
  while(1) {
    ...
    while((big_g.vedge[startedge].endpt >= 0) && ((edgenum-startedge) > 0)) {
      ++startedge;
    }
    ...
    stp = &poly[big_g.vedge[startedge].startpt];
    ...
    edgei = startedge+1;
    while(edgei < edgenum) {
      if(((big_g.vedge[edgei].plane[0] == planeA)&&(big_g.vedge[edgei].plane[1] == planeC)) ||
	 ((big_g.vedge[edgei].plane[0] == planeC)&&(big_g.vedge[edgei].plane[1] == planeA))) {
    ...
    if(edgeflag == 'Y') { // add edge
      add_vedge(big_g.vedge, edgenum, cont, planeA, planeC, planeB, poly, vn);
      ++edgenum;
    }
    ...
    edgei = startedge+1;
    while(edgei < edgenum) {
      if(((big_g.vedge[edgei].plane[0] == planeB)&&(big_g.vedge[edgei].plane[1] == planeC)) ||
	 ((big_g.vedge[edgei].plane[0] == planeC)&&(big_g.vedge[edgei].plane[1] == planeB))) {
	// already on list, add current vertex as endpt
	big_g.vedge[edgei].endpt = vn;
    ...
    if(edgeflag == 'Y') { // add edge
      add_vedge(big_g.vedge, edgenum, cont, planeB, planeC, planeA, poly, vn);
      ++edgenum;
  ...
  for(edgei=0; edgei<edgenum; ++edgei) {
    if((rado < poly[big_g.vedge[edgei].startpt].dist) || (rado < poly[big_g.vedge[edgei].endpt].dist)) {
  ...
  for(edgei=0; edgei<edgenum; ++edgei) {
    if(big_g.vedge[edgei].arc != 'X') {
      if(solve_2xS(cont[big_g.vedge[edgei].plane[0]], cont[big_g.vedge[edgei].plane[1]], rado, arcpt0, arcpt1) == -1) {
	...
	add_vertex(poly, vn, arcpt0, big_g.vedge[edgei].plane[0], big_g.vedge[edgei].plane[1], -1);
	poly[vn].dist = rado;
	++vn;
      }
	...
	+ cont[testpB].Ai[2]*arcpt1[2] + cont[testpB].Ai[3];
      if((testvalA < 0.0) && (testvalB < 0.0)) { // point is good
	add_vertex(poly, vn, arcpt1, big_g.vedge[edgei].plane[0], big_g.vedge[edgei].plane[1], -1);
	poly[vn].dist = rado;
	++vn;
      }
    }
  ...
  if(recalc == 'N') {
    save_seeds(cont, poly, vn, atomzero,big_g);
  } else {
...
void save_areas(struct plane cont[], struct contactlist contlist[], int NC, int atomzero, struct big_global big_g,struct big_global *bla)
{
  ...
  printf("Numcarec end:%ld\n",numcarec);
  bla->numcarec=numcarec;
 // blanumcarec
  return;
}
		  ...
		  int PDBtot, float rado, int dim, struct big_global big_g) 
{
  ...
  while(currindex != -1) {
   ...
   // if (currindex == ca_rec[currindex].prev) {break;}
